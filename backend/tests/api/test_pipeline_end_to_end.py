...
def _run_pipeline_e2e(client, tiny_pdf) -> None:  # noqa: ANN001
    ...
    try:
        ...
        if rels:
            detail = client.get(
                f"/projects/{pid}/relationships/{rels[0]['relationship_id']}"
            ).json()
...
def test_upload_process_and_read_everything_over_http(client, tiny_pdf) -> None:
    _run_pipeline_e2e(client, tiny_pdf)


def test_pipeline_e2e_leaves_no_project_behind(client, tiny_pdf) -> None:
    """Regression: the e2e run above once created its project and never deleted
    it, leaking one row into the database on every run."""
    before = {p["project_id"] for p in client.get("/projects").json()}

    _run_pipeline_e2e(client, tiny_pdf)

    after = client.get("/projects").json()
    assert {p["project_id"] for p in after} == before
    assert not any(p["name"] == "api-e2e" for p in after)
